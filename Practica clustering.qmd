...
datos_complete %>% 
  arrange(Continente)
```
Veamos que países encontramos en cada cluster: en el primer cluster nos encontramos con países de África, Ásia y Oceanía mayoritariamente; el cluster 2 contiene todos los países de Europa, algunos de Ásia, como Qatar o Singapur, y gran parte de los países de Norteamérica y Sudamérica; el tercer cluster contiene mayoritariamente países de Ásia, Sudamérica y Oceanía; finalmente, en el cluster 4 encontramos únicamente  países de África.
...
Para comparar los resultados de los árboles resultantes, usamos la función cophenetic:
```{r}
...
Podemos ver que el dendograma que mejor representa la similitud real entre los individuos es el hecho mediante el método de linkage ward, por lo que nos quedaremos con este.
...
```{r, echo = FALSE, warning=FALSE, message=FALSE}
fviz_cluster(object = list(data = datos_escalados, cluster = cutree(hc_completo, k = 3)), geom = "point", ellipse.type = "convex",repel = TRUE,
